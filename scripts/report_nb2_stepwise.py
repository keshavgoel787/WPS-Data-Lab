...
def _manuscript_docx_status():
    """One sentence on whether the manuscript tables were regenerated, read from
    git rather than asserted.

    This arm does not touch `WPS_Table_Sheels_*.docx`, but a later fix might --
    and did. Reporting a stale 'nothing was regenerated' beside corrected tables
    would actively mislead, so the claim is derived from each file's last commit
    and falls back to a hedge if git is unavailable rather than guessing."""
    import subprocess
    names = ['WPS_Table_Sheels_augmented.docx',
             'WPS_Table_Sheels_augmented_2021.docx',
             'WPS_Table_Sheels_filled_corrected.docx']
    try:
        this = subprocess.run(
            ['git', 'log', '-1', '--format=%cI', '--', 'docs/nb2_stepwise_models.md'],
            capture_output=True, text=True, cwd=ROOT.rstrip('/'), timeout=15)
        if this.returncode != 0 or not this.stdout.strip():
            raise RuntimeError('no commit date for this memo')
        newer = []
        for n in names:
            r = subprocess.run(['git', 'log', '-1', '--format=%cI', '--', f'docs/{n}'],
                               capture_output=True, text=True,
                               cwd=ROOT.rstrip('/'), timeout=15)
            if r.returncode == 0 and r.stdout.strip() > this.stdout.strip():
                newer.append(n)
    except Exception:
        return ('The manuscript `.docx` tables are not produced by this arm; '
                'check their commit dates against this memo before assuming '
                'they match (git was unavailable when this line was written).')
    if not newer:
        return ('No manuscript `.docx` was regenerated by this arm, and none has '
                'changed since this memo was written.')
    verb = 'has' if len(newer) == 1 else 'have'
    noun = 'one' if len(newer) == 1 else {2: 'two', 3: 'three'}.get(len(newer),
                                                                   str(len(newer)))
    return (f'No manuscript `.docx` is produced by this arm, but {noun} {verb} '
            f'since been regenerated by other work '
            f'({", ".join("`" + n + "`" for n in newer)}) -- so those tables and '
            'this memo are not from the same run. That regeneration was the '
            'convergence fix to the log-linear table scripts, which is '
            'unrelated to the models reported here.')
...
def write_memo(res, cc, ladder, vt, ct):
    ...
    viol_gap = (f'violations only has an NB1 entry at {"/".join(viol_present)} '
                ...
                'selected family')
    # Split across several sentences deliberately. An earlier revision packed all
    # of this into one ~6-line sentence with three nested parentheticals, which
    # was correct but close to unreadable in a document written for a PI. The
    # values are still derived; only the assembly changed.
    A('NB2 is nonetheless the least-bad **single** choice across both outcomes. '
      'The comparison that matters is against plain NB1, since that is the only '
      'family the ladder fits for both outcomes, and it runs in opposite '
      'directions:')
    A('')
    # NOT str.capitalize() -- that lowercases the REST of the string, turning
    # "NB1"/"ZINB"/"M1/M3" into "nb1"/"zinb"/"m1/m3". Only the first character
    # may change.
    def _upper1(t):
        return t[:1].upper() + t[1:] if t else t

    A(f'- **Inspections:** NB2 beats NB1 decisively -- by {nb1_bits}. '
      f'({_upper1(insp_gap)}.)')
    A(f'- **Violations:** NB2 costs {viol_bits} against NB1. '
      f'({_upper1(viol_gap)}. That cost is the same number as the '
      f'penalty-table row above, because NB1 is what the penalty is measured '
      f'against for this outcome -- not a gap in coverage.)')
    A('')
    A('So one family has to lose somewhere, and NB2 loses least: it gives up '
      'single-digit-to-low-double-digit AIC on violations to gain more than a '
      'hundred on inspections.')
    A('')
    ...
    A('- This arm covers the 2011-2021 WPS-view window only. It does not fit '
      ...
      'specification.')
    # Derived, not asserted. This line used to read "No manuscript .docx was
    # regenerated" -- true when this arm was built, and false from the moment the
    # 2026-08-31 convergence fix regenerated all three table documents. A
    # hardcoded provenance claim about files this script does not own is exactly
    # the kind that goes stale silently, so it is now read from git.
    A(f'- {_manuscript_docx_status()}')
    A('')
