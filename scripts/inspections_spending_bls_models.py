...
WPS Inspections Analysis

DV: total WPS inspections per state-year (EPA + state inspections)
Mirrors spending_bls_models.py exactly; only the outcome differs.

Five Level-2 spending variables:
  ...
  SPEND_AREA  — spending / state land area in sq miles

Note: spend_bls_variables.csv (Level-2 spending denominators) is identical
      to what spending_bls_models.py produces. This script reads from that
      file rather than rebuilding it, but the BLS data pipeline is retained
      for transparency.
"""
...
print("=" * 70)
print("BLS-NORMALIZED SPENDING MODELS — WPS INSPECTIONS ANALYSIS")
print("=" * 70)
...
echo_long_filtered['inspections'] = pd.to_numeric(echo_long_filtered['inspections'], errors='coerce')
echo_long_filtered = echo_long_filtered.dropna(subset=['inspections'])
...
print("  " + "-" * 74)
for v in spend_vars:
    s = spend_lv2[v].dropna()
    print(f"  {v:<12} {len(s):>14} {s.mean():>12.4f} {s.std():>12.4f} "
          f"{s.min():>12.4f} {s.max():>12.4f}")

# Report BLS coverage (states missing each spending variable)
print("\n  BLS coverage (states missing BLS data):")
for v in spend_z_vars:
    missing = spend_lv2[spend_lv2[v].isna()]['state'].tolist()
    if missing:
        print(f"    {v}: {len(missing)} states missing → excluded from that model")
        print(f"      ({', '.join(missing)})")
    else:
        print(f"    {v}: complete")

# ============================================================
# [3] MERGE INTO MODEL DATASET
# ============================================================
print("\n[3] Merging into model dataset...")
df_full = df_model.merge(spend_lv2[['state'] + spend_vars + spend_z_vars], on='state', how='left')
...
# ============================================================
# [6] STEPWISE MODELS — ONE SPEND VARIABLE AT A TIME
# ============================================================
print("\n" + "=" * 70)
print("STEPWISE MODELS — BLS-NORMALIZED SPENDING VARIABLES (DV = INSPECTIONS)")
print("=" * 70)
...
for vz, label in spend_labels.items():
    ...
    ma = fit_mixedlm(
        f'inspections ~ time + time2 + time3 + {vz}',
    ...
    mb = fit_mixedlm(
        f'inspections ~ time + time2 + time3 + {vz} + {vz}:time',
...
# ============================================================
# [8] COMBINED MODEL — ALL FIVE SPEND VARIABLES
# ============================================================
print("\n" + "=" * 70)
