"""
Labor Intensity & DOL H-2A Covariate Models — WPS Violations Analysis

Zimmerman one-at-a-time stepwise for six new Level-2 state covariates
derived from five new datasets:

  lii_2017_z        — Labor Intensity Index, 2017 Census of Agriculture
  lii_2012_z        — Labor Intensity Index, 2012 Census of Agriculture
                      (r=0.977 with 2017; included for cross-wave comparison)
  dol_workers_cert_z — Mean annual H-2A certified workers, DOL 2011–2019
                       (2013 missing from source; 2014 workers_requested=0
                        recorded as certified-only year — both excluded from mean)
  dol_n_cases_z      — Mean annual H-2A employer cases, DOL 2011–2019
  dol_demand_met_z   — Mean % demand met (certified/requested), DOL 2011–2019
                       (2013 and 2014 excluded: missing/inf)
  pct_flc_z          — % H-2A workers placed via Farm Labor Contractors,
                       2020 (DOL employer-type data; earliest available year;
                       used as structural proxy — does not cover study period)

Note: pct_direct (Owner/Operator %) excluded — r=-0.989 with pct_flc.
      lii_2022 excluded — outside study period (2011–2019).
      dol_workers_cert_z corr with h2a_workers (Yuri 2017): r=0.833.
"""
...
print("=" * 70)
print("LABOR COVARIATE STEPWISE MODELS — WPS VIOLATIONS ANALYSIS")
print("=" * 70)
...
# --- 2a. Labor Intensity Index (2017 and 2012 Census) ---
...
# --- 2b. DOL H-2A Annual Workers (2011–2019, excl. 2013 and 2014) ---
...
# 2013 missing from source; 2014 has workers_requested=0 → demand_met_pct=inf
dol_study = dol1[dol1['year'].between(2011, 2019) & (dol1['year'] != 2013)].copy()
print(f"    Years used for mean: {sorted(dol_study['year'].unique())} (2013 excluded: missing)")

dol_means = dol_study.groupby('state').agg(
    dol_workers_cert=('workers_certified', 'mean'),
    dol_n_cases=('n_cases', 'mean'),
    # Exclude 2014 for demand_met_pct (workers_requested=0 → inf)
    dol_demand_met_pct=('demand_met_pct',
                        lambda x: x[x != np.inf].mean())
).reset_index()
...
d2020 = d2020.rename(columns={
    'pct_Farm Labor Contractor': 'pct_flc',
    'pct_Owner / Operator (Direct)': 'pct_direct',
})
level2 = level2.merge(
...
print("    NOTE: pct_direct excluded (r=-0.989 with pct_flc)")
...
# Report correlations with existing predictors
print("\n  Correlations with existing predictors (from h2a_state_summary_2017):")
h2a = pd.read_csv('/Users/keshavgoel/Research/data/raw/h2a_state_summary_2017.csv')
h2a['state'] = h2a['state_code'].map(STATE_ABBREV_TO_NAME)
corr_df = level2.merge(h2a[['state', 'h2a_workers']], on='state')
print(f"    r(dol_workers_cert, h2a_workers_2017) = {corr_df['dol_workers_cert'].corr(corr_df['h2a_workers']):.4f}")
print(f"    r(lii_2017, lii_2012)                 = {level2['lii_2017'].corr(level2['lii_2012']):.4f}")
...
m0 = fit_mixedlm('violations ~ time + time2 + time3',
...
# ============================================================
print("\n" + "=" * 70)
print("STEPWISE MODELS — LABOR/DOL COVARIATES (DV = VIOLATIONS)")
print("=" * 70)
...
for pred_col, pred_label in predictor_sequence:
    ...
    ma = fit_mixedlm(
        f'violations ~ time + time2 + time3 + {pred_col}',
    ...
    mb = fit_mixedlm(
        f'violations ~ time + time2 + time3 + {pred_col} + {pred_col}:time',
...
# ============================================================
print("\n" + "=" * 70)
print("SUMMARY — BETWEEN-STATE VARIANCE DECOMPOSITION (DV = VIOLATIONS)")
...
combined_formula = ('violations ~ time + time2 + time3 + ' +
                    ' + '.join(z_list) +
